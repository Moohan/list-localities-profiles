...
# It prepares the data required for the HSCP-level map and for the
# locality-level tables. By running this once per HSCP, we avoid
...
# This is a large file, so we only want to read it in once per HSCP
...
# We are careful not to remove objects needed by the locality-level script
# e.g. care_homes, postcode_lkp
rm(
  Clacks_Royal,
  data,
  file,
