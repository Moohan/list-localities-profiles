############################################################################################# .
#                                                                                           #
#                         LOCALITY PROFILES SERVICES MANIPULATION CODE                      #
#                                                                                           #
############################################################################################# .

## Code used to manipulate services data for locality profiles.
# This script is intended to be run at the HSCP level to prepare data markers
# for both the HSCP map and the locality-specific tables.
...
markers_care_home_other <- care_homes |>
  select(
    type = care_service,
    subtype,
    name = service_name,
...
# Housekeeping ----
# Remove large lookup and temporary objects that are no longer needed
rm(
  care_homes,
  Clacks_Royal,
  data,
