...
# This script contains HSCP-level logic and should be sourced in an outer loop
# to avoid redundant data loading and processing for every locality.
...
# Locality lookup (datazone level)
# Use a specific name to avoid overwriting the global 'lookup' object
dz_lookup_services <- read_in_localities(dz_level = TRUE)

# Lookup without datazones (locality level)
