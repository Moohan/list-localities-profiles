...
## This script contains HSCP-level data manipulation.
...
## Determine HSCP
# This supports both the full build (where HSCP is defined)
# and single-chapter testing (where only LOCALITY may be defined)
...
# Subset care which is not Elderly care for table (HSCP level)
markers_other_care <- care_homes %>%
  dplyr::select(
    type = care_service,
    subtype,
    name = service_name,
