...
## Code used to manipulate services data for locality profiles at the HSCP level.
## This script is sourced once per HSCP in the outer loop of Build Profiles.R.
...
# Locality lookup (HSCP level)
# Use a specific name for the DZ lookup to avoid overwriting the global 'lookup' variable
...
for (file in services_file_names) {
  ...
  data <- readRDS(
    fs::path(lp_path, paste0("Services/DATA ", ext_year), file)
  ) |>
    janitor::clean_names()
