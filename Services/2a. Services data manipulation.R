...
## Code used to manipulate services data for locality profiles at HSCP level.
## This script should be sourced once per HSCP in the outer loop.
...
## Determine HSCP defensively (supports single-locality testing)
...
services_file_names <- list.files(services_data_path, pattern = "RDS")

for (file in services_file_names) {
  name <- substr(x = file, 1, 4)

  data <- readRDS(fs::path(services_data_path, file)) |>
    janitor::clean_names()
...
# Housekeeping ----
# These intermediate objects are not needed by downstream scripts (Map or Table)
rm(
  data,
  file,
  services_file_names,
