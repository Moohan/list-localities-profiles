############################################################################################# .
#                                                                                           #
#                   LOCALITY PROFILES SERVICES DATA MANIPULATION (HSCP)                     #
#                                                                                           #
############################################################################################# .

# This script reads in and manipulates services data at the HSCP level.
# It is designed to be run once per HSCP to avoid redundant data processing.
# It creates the `markers_*` data frames needed for both the HSCP-level map
# and the locality-level tables.

###### 1. Set up ######

# The `HSCP` variable is expected to be available from the calling environment
# (the outer loop in `Locality Profiles Render Code.R`).

# Change year to be the year in the data folder name
ext_year <- 2024

# Set file path (already available from Global Script)
# lp_path <- "/conf/LIST_analytics/West Hub/02 - Scaled Up Work/RMarkdown/Locality Profiles/"

### Geographical lookups and objects ----

# Locality lookup (without datazones)
# This is used for merging and filtering data by HSCP.
...
# Housekeeping ----
# These objects are no longer needed after the markers have been created.
rm(
  prac,
  hosp_lookup,
  services_file_names,
