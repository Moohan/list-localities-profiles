...
# This script processes services data at the HSCP level.

# Lookup without datazones
lookup2 <- read_in_localities()

## Determine HSCP if not already defined (e.g. for testing)
if (!exists("HSCP")) {
...
hosp_postcodes_processed <- dplyr::rename(hosp_postcodes, location = hospital_code)
...
hosp_lookup <- hosp_types |>
  ...
  dplyr::left_join(
    dplyr::select(hosp_postcodes_processed, location, postcode),
...
rm(hosp_lookup, hosp_postcodes_processed)
