...
hosp_lookup <- hosp_types |>
  ...
  dplyr::left_join(
    dplyr::select(hosp_postcodes_renamed, location, postcode),
...
clacks_royal_marker <- dplyr::filter(hosp_lookup, name == "Forth Valley Royal Hospital")
...
rm(prac_filtered, hosp_postcodes_renamed, hosp_lookup, clacks_royal_marker)
