...
## This script contains the HSCP-level logic that is run once per HSCP.

## Written by C.Puech
## Created on 24/02/2020
## Latest update August 2022 - rewrote parts of code for smoother process
## Refactored by Bolt - 2024-07-29 - Moved HSCP-level logic to improve performance.
...
# The `lookup` object (without datazones) is already loaded in `Build Profiles.R`.
# The `HSCP` variable is also available from the outer loop in that script.
# Script #3 needs `lookup2`, so we create it here from `lookup`.
lookup2 <- lookup
