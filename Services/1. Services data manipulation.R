############################################################################################# .
#                                                                                           #
#                         LOCALITY PROFILES SERVICES DATA PREPARATION                       #
#                                                                                           #
############################################################################################# .

## Code used to manipulate services data for locality profiles.

## This script prepares data at the HSCP level to avoid re-processing for each locality.
...
rm(curr, hosp, MDSF, name, data, file, services_file_names)
...
# Subset care which is not Elderly care for table - needed in the next script
...
# Housekeeping
# The objects created here are used in the locality loop.
# They will be cleaned up at the end of the HSCP loop in Build Profiles.R
rm(hosp_postcodes, hosp_types, prac, hosp_lookup, Clacks_Royal, postcode_lkp)
