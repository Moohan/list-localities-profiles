...
house_dat_hscp <- house_raw_dat |>
  ...
  dplyr::ungroup() |>
  dplyr::mutate(
    perc_discount = janitor::round_half_up(
...
# 3. All Locality Totals in HSCP (to be subset for 'other localities' later) ----

# This pre-calculates for all localities in the HSCP, so we don't have to re-calculate inside the locality loop.
...
house_dat_all_locs <- house_raw_dat |>
  ...
  dplyr::ungroup() |>
  dplyr::mutate(
    tax_discount_perc = janitor::round_half_up(
      tax_discount / total_dwellings * 100,
      1
    ),
    tot_dwellings_chr = format(total_dwellings, big.mark = ",")
  )
