...
for (HSCP in hscp_list) {
  ...
  # Services (Partnership level) ----
  # Hoist data manipulation and map generation to the outer loop to improve performance.
  # These are sourced once per HSCP rather than once per locality.
  ...
  # Clean up objects created in the outer loop that are no longer needed.
  rm(
    list = intersect(
      c(
        "service_map",
