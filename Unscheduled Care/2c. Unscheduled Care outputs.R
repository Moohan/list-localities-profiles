...
other_loc_emergency_adm <- emergency_adm_hscp_aggregates %>%
  ...
  left_join(
    pop_hscp_aggregates %>%
      filter(area_type == "Locality", age_group == "Total"),
...
first_ea_max_age <- emergency_adm_age %>%
  filter(year == min(year), age_group == age_group_max_ea) %>%
  pull(data)
...
first_ea_min_age_dat <- emergency_adm_age %>%
  filter(age_group == age_group_min_ea, year == min(year))
...
first_ubd_max_age <- bed_days_age %>%
  filter(year == min(year), age_group == age_group_max_ubd) %>%
  pull(data)
...
first_ubd_min_age_dat <- bed_days_age %>%
  filter(age_group == age_group_min_ubd, year == min(year))
...
first_bd_mh_max_age <- bed_days_mh_age %>%
  filter(year == min(year), age_group == age_group_max_mh) %>%
  pull(data)
...
first_bd_mh_min_age_dat <- bed_days_mh_age %>%
  filter(age_group == age_group_min_mh, year == min(year))
...
first_bed_days_mh_loc <- bed_days_mh_areas %>%
  filter(location == LOCALITY, year == min(year)) %>%
  pull(data)
...
first_hscp_bed_days_mh <- bed_days_mh_areas %>%
  filter(location == HSCP, year == min(year)) %>%
  pull(data)
...
first_scot_bed_days_mh <- bed_days_mh_areas %>%
  filter(location == "Scotland", year == min(year)) %>%
  pull(data)
...
first_fy_hb_mh <- filter(
  bed_days_mh_areas,
  financial_year == min_year_bd_mh_areas,
  location == HB
)$data
...
first_ae_att_max_age <- ae_att_age %>%
  filter(year == min(year), age_group == age_group_max) %>%
  mutate(formatted_data = format(data, big.mark = ","))
...
first_ae_att_min_age <- ae_att_age %>%
  filter(year == min(year), age_group == age_group_min) %>%
  mutate(formatted_data = format(data, big.mark = ","))
...
first_fy_rate_ae_areas <- filter(
  ae_att_areas,
  financial_year == min_year_ae_area,
  location == LOCALITY
)$data
...
hscp_dd_loc <- delayed_disch_areas %>%
  filter(location == HSCP, year == max(year)) %>%
  mutate(formatted_data = format(data, big.mark = ","))
...
scot_dd_loc <- delayed_disch_areas %>%
  filter(location == "Scotland", year == max(year)) %>%
  mutate(formatted_data = format(data, big.mark = ","))
...
other_loc_dd <- delayed_disch_hscp_aggregates %>%
  filter(area_type == "Locality", location %in% other_locs$hscp_locality) %>%
  left_join(
    pop_hscp_aggregates %>% filter(area_type == "Locality", age_group == "65+"),
    by = join_by(financial_year, location)
  ) %>%
  filter(year == max(year)) %>%
  mutate(data = format(round_half_up(n / pop * 100000), big.mark = ",")) %>%
...
first_read_loc_dat <- readmissions_areas %>%
  filter(location == LOCALITY, year == min(year))
first_read_loc1 <- first_read_loc_dat %>% pull(data)
latest_read_loc_dat <- readmissions_areas %>%
  filter(location == LOCALITY, year == max(year))
...
hb_read_loc <- readmissions_areas %>%
  filter(location == HB, year == max(year)) %>%
  mutate(formatted_data = format(data, big.mark = ","))
...
first_fy_hb_read <- filter(
  readmissions_areas,
  financial_year == min_year_re_area,
  location == HB
)$data
...
ppa_65plus <- ppa_65plus_hscp_aggregates %>%
  filter(location %in% c(LOCALITY, HSCP, HB, "Scotland")) %>%
  left_join(
    ppa_hscp_aggregates %>% select(financial_year, location, n),
    by = join_by(financial_year, location)
  ) %>%
  left_join(pop_areas_all_ages, by = join_by(financial_year, location)) %>%
...
latest_ppa_loc_dat <- ppa_areas %>%
  filter(location == LOCALITY, year == max(year) | year == min(year)) %>%
  mutate(formatted_data = format(data, big.mark = ","))
latest_ppa_loc1 <- latest_ppa_loc_dat$formatted_data[2]
ppa_diff <- percent_change_calc(
  latest_ppa_loc_dat$data[2],
  latest_ppa_loc_dat$data[1]
)
...
hscp_ppa_loc <- ppa_areas %>%
  filter(location == HSCP, year == max(year) | year == min(year)) %>%
  mutate(formatted_data = format(data, big.mark = ","))
ppa_diff_hscp <- percent_change_calc(hscp_ppa_loc$data[2], hscp_ppa_loc$data[1])
ppa_word_change_hscp <- word_change_calc(
  hscp_ppa_loc$data[2],
  hscp_ppa_loc$data[1]
)
hscp_ppa <- hscp_ppa_loc # for Rmd compatibility

scot_ppa_loc <- ppa_areas %>%
  filter(location == "Scotland", year == max(year) | year == min(year)) %>%
  mutate(formatted_data = format(data, big.mark = ","))
diff_scot_ppa <- percent_change_calc(scot_ppa_loc$data[2], scot_ppa_loc$data[1])
word_change_scot_ppa <- word_change_calc(
  scot_ppa_loc$data[2],
  scot_ppa_loc$data[1]
)
scot_ppa <- scot_ppa_loc # for Rmd compatibility

hb_ppa_loc <- ppa_areas %>%
  filter(location == HB, year == max(year) | year == min(year)) %>%
  mutate(formatted_data = format(data, big.mark = ","))
...
latest_period_psych_hosp <- unique(
  filter(psych_hosp_raw, year == max(year))$period_short
)
...
psych_hosp_latest <- round_half_up(
  filter(
    psych_hosp_raw,
    year == max(year) & (area_name == LOCALITY & area_type == "Locality")
  )$measure,
  1
)
hscp_psych_hosp_val <- round_half_up(
  filter(
    psych_hosp_raw,
...
scot_psych_hosp_val <- round_half_up(
  filter(psych_hosp_raw, year == max(year) & area_name == "Scotland")$measure,
  1
)
