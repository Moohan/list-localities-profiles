...
emergency_adm_raw <- read_parquet(paste0(import_folder, "emergency_admissions_msg.parquet")) %>%
  filter(financial_year <= max_fy)

bed_days_raw <- read_parquet(paste0(import_folder, "bed_days_msg.parquet")) %>%
  filter(financial_year <= max_fy)

bed_days_mh_raw <- read_parquet(paste0(import_folder, "bed_days_mh_msg.parquet")) %>%
  filter(financial_year <= max_fy)

ae_attendances_raw <- read_parquet(paste0(import_folder, "ae_attendances_msg.parquet")) %>%
  filter(financial_year <= max_fy)

delayed_disch_raw <- read_parquet(paste0(import_folder, "delayed_discharges_msg.parquet")) %>%
  filter(financial_year <= max_fy) %>%
...
readmissions_raw <- read_parquet(paste0(import_folder, "readmissions_smr.parquet")) %>%
  filter(financial_year <= max_fy)

ppa_raw <- read_parquet(paste0(import_folder, "ppa_smr.parquet")) %>%
  filter(financial_year <= max_fy)

psych_hosp_raw <- read_csv(paste0(import_folder, "scotpho_data_extract_psychiatric_admissions.csv")) %>%
  clean_scotpho_dat() %>%
...
# compute age bands
populations_raw$"Pop0_17" <- rowSums(subset(populations_raw, select = age0:age17))
populations_raw$"Pop18_44" <- rowSums(subset(populations_raw, select = age18:age44))
populations_raw$"Pop45_64" <- rowSums(subset(populations_raw, select = age45:age64))
populations_raw$"Pop65_74" <- rowSums(subset(populations_raw, select = age65:age74))
populations_raw$"Pop75Plus" <- rowSums(subset(populations_raw, select = age75:age90plus))
populations_raw$"Pop65Plus" <- rowSums(subset(populations_raw, select = age65:age90plus))
